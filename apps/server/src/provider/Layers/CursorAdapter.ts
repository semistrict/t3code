...
export interface CursorAdapterLiveOptions {
  ...
  >;
  readonly configureRuntime?: (input: {
  ...
  readonly beforeInterrupt?: (input: {
    readonly runtime: AcpSessionRuntime.AcpSessionRuntime["Service"];
    readonly threadId: ThreadId;
  ...
  return Effect.gen(function* () {
      ...
      Effect.mapError(
        (cause) =>
          new ProviderAdapterRequestError({
            ...
            detail: `Failed to generate ${runtimeName} runtime identifier.`,
            cause,
          }),
      ),
    );
        ...
        Effect.mapError(
          (cause) =>
            new EffectAcpErrors.AcpTransportError({
              detail: `Failed to process ${runtimeName} ACP extension event.`,
              cause,
            }),
        ),
      );
      ...
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          const started = yield* Effect.gen(function* () {
            if (options?.includeCursorExtensions !== false) {
            ...
            if (options?.configureRuntime) {
              yield* options.configureRuntime({
                runtime: acp,
                threadId: input.threadId,
          ...
          const nf = yield* Stream.runDrain(
            ...
            Effect.catch((cause) =>
              Effect.logError(`Failed to process ${runtimeName} runtime notification.`, { cause }),
            ),
          ...
          yield* offerRuntimeEvent({
            ...
            provider: provider,
            threadId: input.threadId,
            payload: { state: "ready", reason: `${runtimeName} ACP session ready` },
          });
          yield* offerRuntimeEvent({
            type: "thread.started",
            ...(yield* makeEventStamp()),
    ...
    const interruptTurn: CursorAdapterShape["interruptTurn"] = (threadId) =>
      Effect.gen(function* () {
        ...
        if (options?.beforeInterrupt && sessionId) {
          yield* Effect.ignore(options.beforeInterrupt({ runtime: ctx.acp, threadId, sessionId }));
        }
        yield* Effect.ignore(
          ctx.acp.cancel.pipe(
            Effect.mapError((error) =>
    ...
    yield* Effect.addFinalizer(() =>
      Effect.forEach(sessions.values(), stopSessionInternal, { discard: true }).pipe(
        Effect.catch((cause) =>
          Effect.logError(`Failed to emit ${runtimeName} session shutdown event.`, { cause }),
        ),
